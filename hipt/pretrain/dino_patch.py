    ...
    with tqdm.tqdm(
        ...
        for epoch in t:
            ...
            our_state_dict =  {
                ...
                "cfg.training.clip_grad": cfg.training.clip_grad,
                "cfg.training.freeze_last_layer": cfg.training.freeze_last_layer,
            }
            our_save_path = Path(snapshot_dir, "our", f"epoch_{epoch:03}.pt")
            os.makedirs(Path(snapshot_dir, "our"), exist_ok=True)
            torch.save(our_state_dict, our_save_path)
            #####################################    
            if cfg.wandb.enable and is_main_process():
