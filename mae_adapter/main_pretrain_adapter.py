...
# ##loading previous data when loading from a saved model part1
# wandb.init(project="dino_adapter", id='0ruzu8nr')
...
def get_args_parser():
    ...
    parser.add_argument('--train_linears__linears_from_scratch', type=list, default='[1, 1]', metavar='N', help='for sweep and wether train the head linears of encoder and decoder or not and whether train linears from scratch or not')
...
def main(args):
    ...
    if True:  # args.distributed:
        ...
        sampler_train = torch.utils.data.DistributedSampler(
            dataset_train, num_replicas=num_tasks, rank=global_rank, shuffle=True #ye check konam bebinam in suffle dare ya na
        )
        sampler_valid = torch.utils.data.DistributedSampler(
            dataset_valid, num_replicas=num_tasks, rank=global_rank, shuffle=False #ye check konam bebinam in suffle dare ya na
        )
    ...
    if args.finetune == 1:
        ...
        for k in ['decoder_embed.weight', 'decoder_embed.bias', 'decoder_pred.weight', 'decoder_pred.bias']:
            if k in checkpoint_model and (checkpoint_model[k].shape != state_dict[k].shape or (args.train_linears__linears_from_scratch[1] == 1)):
    ...
    for name, p in model.named_parameters():
        ...
        if (bool(args.train_linears__linears_from_scratch[0])) and (name in ['decoder_embed.weight', 'decoder_embed.bias', 'decoder_pred.weight', 'decoder_pred.bias']):
            p.requires_grad = True
    ...
    #wandb resume (this may be a little bit buggy but we probably won't even need it)
    if args.resume != '':
        start_epoch = args.start_epoch - 1 #? or start_epoch = args.start_epoch
        cnt = 0
        #for train
...
def train_one_epoch(model: torch.nn.Module,
    ...
    # metric_logger_valid.add_meter('lr', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = 'Valid Epoch: [{}]'.format(epoch)
    
    with torch.no_grad():
        for data_iter_step, (samples, _) in enumerate(metric_logger_valid.log_every(data_loader_valid, print_freq, header)):
    ...
    return {k: meter.global_avg for k, meter in metric_logger_train.meters.items()}, sum(metrics_dict_valid["step_loss"]) / len(metrics_dict_valid["step_loss"])
