...
from datasets.dsmil_dataset import DSMILDataset
...
parser.add_argument('--task', type=str, choices=[
    'task_1_tumor_vs_normal', 'task_2_tumor_subtyping', 'task_camelyon16_dsmil_feats'
])
...
elif args.task == 'task_camelyon16_dsmil_feats':
    args.n_classes = 2
    args.k = 1
    args.weighted_sample = False
    dataset = DSMILDataset()
else:
...
if __name__ == "__main__":
    if args.wandb_run is None:
        args.wandb_run = 'Experiment'
    wandb.init(
        project=f"CLAM_{args.task}",
        name=args.wandb_run,
        config={**vars(args)},
