...
def crop_slide(img, save_slide_path, position=(0, 0), step=(0, 0), patch_size=256): # position given as (x, y)
        ...
        img = np.array(img)[..., :3]
        if thres_saturation(img, 15):
            patch_name = "{}_{}".format(step[0], step[1])
            io.imsave(join(save_slide_path, patch_name + ".jpg"), img_as_ubyte(img))

def slide_to_patch(out_base, img_slides, step, folder='test', patch_size=256):
...
if __name__ == '__main__':
    ...
    slide_to_patch(out_base, all_slides, step, folder, args.patch_size)