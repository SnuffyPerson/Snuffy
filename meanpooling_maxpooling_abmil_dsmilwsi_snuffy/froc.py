#commad: python evaluate.py --reference=reference.csv --masks=masks --detections=detections --plot
"""
...
def area_under_curve(x, y):
    """
    Calculates the area under the curve given two arrays of x and y values.
    """
    # print(y, x)
    return np.trapz(y, x)
...
def mp_compute_froc(froc_data, num_processes):
    ...
    with mp.Pool(num_processes) as pool:
        total_fps, total_tps = zip(*pool.map(mp_greater_equal, [(aggregated_fps, aggregated_tps, threshold) for threshold in all_probs + [-1]]))
    # print(f'--------------------------------------------------mp: {time() - t}')
...
def computeFROC(reference_file_path, masks_folder_path, detections_folder_path, result_file_path,
 include_itcs, plot_curve, evaluation_mask_level, images_to_calculate_for, plot_path):
    ...
    # Compute FROC.
    #
    average_fps, sensitivities, thresholds = compute_froc(froc_data=froc_data)
    ...
    #
    if plot_curve:
        plot_froc(average_fps=average_fps, sensitivities=sensitivities, path=plot_path) #make it save if it doesn't

    trapz_froc = area_under_curve(average_fps, sensitivities)
    print(f'FROC: {trapz_froc}')
    return challenge_score, trapz_froc
...
def mp_froc_data(args):
    image_name, masks_folder_path, detections_path, evaluation_mask_level, include_itcs, slide_type = args
    mask_path = os.path.join(masks_folder_path, '{image}_mask.tif'.format(image=image_name))
    detection_path = os.path.join(detections_folder_path, '{image}.csv'.format(image=image_name))
    print('Processing: {path}'.format(path=detection_path))
    detection_items = load_detections(detection_path=detection_path, mask_path=mask_path, level=evaluation_mask_level)
    evaluation_mask, itc_labels = compute_evaluation_mask(mask_path=mask_path, level=evaluation_mask_level, include_itcs=include_itcs) if slide_type == 'tumor' else (None, set())
    fp_probs, tp_probs, number_of_tumors = compute_probabilities(detection_items=detection_items, evaluation_mask=evaluation_mask, itc_labels=itc_labels)
    return fp_probs, tp_probs, number_of_tumors
...
def mp_computeFROC_list_no_cache(reference_file_path, masks_folder_path, detections, result_file_path,
 include_itcs, plot_curve, evaluation_mask_level, images_to_calculate_for, plot_path, plot_prefix, num_processes):
    global use_cache
    use_cache = False
    """Entry point. Calculates and plots the FROC curve."""
    ...
    for _, reference_row in reference_table.iterrows():
        ...
        if image_name.split('.')[0] not in images_to_calculate_for:
            continue
        mp_list.append((image_name, masks_folder_path, detections, evaluation_mask_level, include_itcs, reference_row['type']))
    with mp.Pool(num_processes) as pool:
    ...
    #
    if plot_curve:
        plot_froc(average_fps=average_fps, sensitivities=sensitivities, path=plot_path, plot_prefix=plot_prefix) #make it save if it doesn't
