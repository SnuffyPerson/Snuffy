...
def train(train_df, milnet, criterion, optimizer, args):
    ...
    for i in range(len(train_df)):
        ...
        ins_prediction, bag_prediction, _, _, avg_prediction = milnet(bag_feats)
        bag_loss = criterion(bag_prediction.view(1, -1), bag_label.view(1, -1))
        avg_loss = criterion(avg_prediction.view(1, -1), bag_label.view(1, -1))
        loss = 0.5*bag_loss + 0.5*avg_loss
        #
        bag_prediction = (bag_prediction+avg_prediction)/2
        bag_prediction = torch.sigmoid(bag_prediction)
...
def dropout_patches(feats, p):
    ...
    sampled_feats = np.unique(sampled_feats, axis=0)
...
def test(test_df, milnet, criterion, args):
    ...
    with torch.no_grad():
        for i in range(len(test_df)):
            ...
            if args.average:
                test_predictions.extend([(0.5*torch.sigmoid(avg_prediction)+0.5*torch.sigmoid(bag_prediction)).squeeze().cpu().numpy()])
            else: test_predictions.extend([(0.0*torch.sigmoid(avg_prediction)+1.0*torch.sigmoid(bag_prediction)).squeeze().cpu().numpy()])
...
def main():
    ...
    wandb.init(
      ...
      project="tcga dsmil average attention",
    
      # track hyperparameters and run metadata
      config={
        "learning_rate": args.lr,
        "architecture": "DSMIL average",
        "dataset": "TCGA",
        "epochs": args.num_epochs,
        "weight_decay": args.weight_decay,
        "split": args.split,
        "dropout patch": args.dropout_patch,
        "dropout node": args.dropout_node
      }
    )
    
    if args.model == 'dsmil':
        import average_pooling as mil
    ...
    for epoch in range(1, args.num_epochs):
        ...
        wandb.log({"train loss": train_loss_bag, "test loss": test_loss_bag, "auc": aucs[0], "average score": avg_score})
    wandb.finish()
