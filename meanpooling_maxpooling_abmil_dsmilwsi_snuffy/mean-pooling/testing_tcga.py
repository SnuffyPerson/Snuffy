    ...
    for i in range(0, num_bags):
        ...
        with torch.no_grad():
            for iteration, batch in enumerate(dataloader):
                ...
                feats, classes, _ = milnet.i_classifier(patches)
            ...
            avg_predicition = np.mean(classes_arr, axis=0)
            avg_predicition = torch.from_numpy(avg_predicition).cuda()
            bag_prediction, A, _ = milnet.b_classifier(bag_feats, avg_predicition)
            if args.average:
                bag_prediction = (bag_prediction+avg_predicition)/2
            bag_prediction = torch.sigmoid(bag_prediction).squeeze().cpu().numpy()
            color = [0, 0, 0]
            attentions = torch.zeros_like(A[:, 1])
...
if __name__ == '__main__':
    ...
    state_dict_weights = torch.load(os.path.join('weights', '06042023', '5.pth'))
