...
def softmax_one(x, dim, dtype):
    ...
    ret = torch.div(e_x, sum_e_x.add_(1))
    return ret
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer, c, top_k_indices, random_indices, mode):
        ...
        if mode == 'attn':
            ...
            random_k = torch.index_select(x, dim=1, index=random_indices) if random_indices !=None else None
            top_k = torch.cat((top_ks, random_k), dim=1) if random_indices != None else top_ks
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, c):
        ...
        random_indices = torch.from_numpy(
            np.random.choice(remaining_indices, randoms_share, replace=False)).cuda() if randoms_share!=0 else None
        
        random_k = torch.index_select(x, dim=1, index=random_indices) if randoms_share!=0 else None
        top_k = torch.cat((top_ks, random_k), dim=1) if randoms_share!=0 else top_ks
        x_k, attentions = self.sublayer[0](x, lambda x: self.self_attn(x, top_k, x), c, top_k_share_indices,
                                         random_indices, 'attn')
                         
        selected_indices = torch.hstack((top_k_share_indices, random_indices)) if randoms_share!=0 else top_k_share_indices
        y = x.clone()
        y[:, selected_indices, :] = x_k
...
def attention(query, key, value, use_softmax_one, dropout=None):  # mikham bi mask, input behesh bayad topk bashe
    "Compute 'Scaled Dot Product Attention'"
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)
    if use_softmax_one == 1:
        p_attn = softmax_one(scores, dim=-1, dtype=scores.dtype)
    else:
...
class MultiHeadedAttention(nn.Module):  # ye joor be in bayad query, key matloob bedam
    ...
    def __init__(self, h, d_model, use_softmax_one, dropout=0.1):
        ...
        self.attn = None
        self.use_softmax_one = use_softmax_one
    ...
    def forward(self, query, key, value):
        ...
        x, self.attn = attention(  # be in topk bedam
            query, key, value, self.use_softmax_one, dropout=self.dropout
        )
...
class PositionwiseFeedForward(nn.Module):  # mikham
    ...
    def __init__(self, d_model, d_ff, activation, dropout=0.1):
        ...
        activation_dictionary = {
            'relu': nn.ReLU(),
            'gelu': nn.GELU(),
            'leakyrelu': nn.LeakyReLU(), #no exactly sure about slope
            'selu': nn.SELU() #curretnly don't know about inplace parameter of selu
        }
