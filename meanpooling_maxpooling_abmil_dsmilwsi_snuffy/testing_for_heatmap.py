...
def test(args, bags_list, milnet):
    ...
    for i in range(0, num_bags):
        ...
        with torch.no_grad():
            ...
            # figure_path = os.path.join(slide_output_path, f'dsmil' + '.png')
            # visualize(attentions, pos_arr, color, figure_path)
            # figure_path = os.path.join(slide_output_path, f'dsmil_blended' + '.png')
            # blend_and_visualize(attentions, pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap)

            # for head in range(4):
            #    attentions = A[0, head, :, :]
            #    attentions = attentions.mean(dim=1)
            #    figure_path = os.path.join(slide_output_path, f'_head_{head}' + '.png')
            #    visualize(attentions, pos_arr, color, figure_path)
            #    if get_mask:
            #        figure_path = os.path.join(slide_output_path, f'_head_{head}_blended' + '_mask.png')
            #    else:
            #        figure_path = os.path.join(slide_output_path, f'_head_{head}_blended' + '.png')
            #    blend_and_visualize(attentions, pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap)

            # ______________________ Avg(Head).Avg(k) ______________________
            # attentions = A.mean(dim=-1).mean(dim=1).squeeze()
            # figure_path = os.path.join(slide_output_path, f'avg(head(avg(k))' + '.png')
            # visualize(attentions, pos_arr, color, figure_path)
            # figure_path = os.path.join(slide_output_path, f'avg(head(avg(k))_blended' + '.png')
            # blend_and_visualize(attentions, pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap)

            # ______________________ InsPred ______________________
            # figure_path = os.path.join(slide_output_path, f'_inspred.png')
            # visualize(ins_classes.squeeze(), pos_arr, color, figure_path)
            # figure_path = os.path.join(slide_output_path, f'_inspred_blended.png')
            # blend_and_visualize(ins_classes.squeeze(), pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap)

            # ______________________ Sigmoid(InsPred) ______________________
            # attentions = ins_classes.squeeze()
            # figure_path = os.path.join(slide_output_path, f'_sigmoid_inspred.png')
            # visualize(torch.sigmoid(ins_classes.squeeze()), pos_arr, color, figure_path)
            # figure_path = os.path.join(slide_output_path, f'_sigmoid_inspred_blended.png')
            # blend_and_visualize(torch.sigmoid(ins_classes.squeeze()), pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap)
            ...
            for cmap in ['jet']:
                ...
                # figure_path = os.path.join(slide_output_path, f'sigmoid_{cmap}.png')
                # blend_and_visualize(torch.sigmoid(ins_classes.squeeze()), pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap, save_wsi=save_wsi)
                # save_wsi = False
                # figure_path = os.path.join(slide_output_path, f'inverted_{cmap}.png')
                # blend_and_visualize(ins_classes.squeeze(), pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap, invert=True)
                # figure_path = os.path.join(slide_output_path, f'inverted_sigmoid_{cmap}.png')
                # blend_and_visualize(torch.sigmoid(ins_classes.squeeze()), pos_arr, color, figure_path, input_slide, alpha, x, y, input_image_size, dpi, get_mask, input_mask, cmap=cmap, invert=True)
                #save_colorbar(cmap, os.path.join(slide_output_path, '_colorbar.png'))
...
def plot_umap(args, milnet):
    x_mean, y = get_dimentionality_reduction_data(args, milnet)
    os.makedirs(os.path.join('test-c16', 'output', 'all', 'umap'), exist_ok=True)
    metrics = ['euclidean', 'manhattan', 'chebyshev', 'cosine', 'correlation', 'minkowski', 'canberra', 'braycurtis',
               'mahalanobis', 'wminkowski', 'seuclidean']
    metrics = ['euclidean', 'manhattan', 'chebyshev', 'cosine', 'correlation', 'minkowski', 'canberra', 'braycurtis',
               'mahalanobis', 'wminkowski']
    for min_dist_ in range(2, 5):
        min_dist = min_dist_ / 10
        for metric in metrics:
            for n in tqdm(range(2, 129)):
                u_map = umap.UMAP(n_components=2, n_neighbors=n, min_dist=min_dist, metric=metric)
                feats = u_map.fit_transform(x_mean)
                x1 = feats[:, 0]
                x2 = feats[:, 1]
                plt.figure()
                plt.scatter(x1[y == 0], x2[y == 0], s=3, c='r')
                plt.scatter(x1[y == 1], x2[y == 1], s=3, c='b')
                os.makedirs(os.path.join('test-c16', 'output', 'all', 'umap', metric, f'min_dist_{min_dist}'),
                            exist_ok=True)
                plt.savefig(os.path.join('test-c16', 'output', 'all', 'umap', metric, f'min_dist_{min_dist}',
                                         f'neighbors_{n}.png'))
                plt.close()


def test_tSNE(args, milnet):
    x_means, y = get_dimentionality_reduction_data(args, milnet)
    t = time.time()
    # t-SNE
    print('fitting t-SNE')
    tsne = TSNE(n_components=3, learning_rate='auto', init='pca', perplexity=args.perplexity, method='exact',
                n_iter=10000)
    tsne_features = tsne.fit_transform(x_means)
    print(f'fitted in {(time.time() - t):.1f}s')
    tSNE_path = os.path.join('test-c16', 'output', 't-SNE.png')
    # 2D plotting
    # x1 = tsne_features[:, 0]
    # x2 = tsne_features[:, 1]
    # plt.scatter(x1[y==0], x2[y==0], s=3, c='r')
    ...
    # x2 = tsne_features[:, 1]
    # x3 = tsne_features[:, 2]
    # ax.scatter(x1[y==0], x2[y==0], x3[y==0], c='r')
    # ax.scatter(x1[y==1], x2[y==1], x3[y==1], c='b')
    # plt.savefig(tSNE_path)
    ...
    #    'label': y
    # })
    # fig = px.scatter_3d(df, x='x1', y='x2', z='x3', color='label')
    # fig.write_html(os.path.join('test-c16', 'output', 't-SNE.html'))

    # for i in tqdm(range(1, 129)):
    #    #tsne = TSNE(n_components=3, learning_rate='auto', init='pca', perplexity=i, method='exact', n_iter=5000)
    #    #tsne_features = tsne.fit_transform(x_means)
    #    #x1 = tsne_features[:, 0]
    #    #x2 = tsne_features[:, 1]
    #    #x3 = tsne_features[:, 2]
    ...
    #    #})
    #    #fig = px.scatter_3d(df, x='x1', y='x2', z='x3', color='label')
    #    #fig.write_html(os.path.join('test-c16', 'output', 'all', '3D', f't-SNE_preplexity_{i}.html'))
    #    lr = 'auto'
    #    tsne = TSNE(n_components=2, learning_rate=lr, init='random', perplexity=i, method='exact', n_iter=10000, early_exaggeration=1000)
    #    tsne_features = tsne.fit_transform(x_means)
    #    x1 = tsne_features[:, 0]
    #    x2 = tsne_features[:, 1]
    #    plt.figure()
    #    plt.scatter(x1[y==0], x2[y==0], s=3, c='r')
    #    plt.scatter(x1[y==1], x2[y==1], s=3, c='b')
    #    #os.makedirs(os.path.join('test-c16', 'output', 'all', f'2D_{lr}'), exist_ok=True)
    #    plt.savefig(os.path.join('test-c16', 'output', 'all', f'2D', f't-SNE_preplexity_{i}.png'))
    #    plt.close()

    for lr in [10, 50]:
        print(f'lr: {lr}')
        for exaggeration in range(200, 1001, 100):
            print(f'exaggeration: {exaggeration}')
            for preplexity in tqdm(range(1, 129)):
                tsne = TSNE(n_components=2, learning_rate=lr, init='pca', perplexity=preplexity, method='exact',
                            n_iter=5000, early_exaggeration=exaggeration)
                tsne_features = tsne.fit_transform(x_means)
                x1 = tsne_features[:, 0]
                x2 = tsne_features[:, 1]
                plt.figure()
                plt.scatter(x1[y == 0], x2[y == 0], s=3, c='r')
                plt.scatter(x1[y == 1], x2[y == 1], s=3, c='b')
                os.makedirs(os.path.join('test-c16', 'output', 'all', '2D', f'lr_{lr}', f'ex_{exaggeration}', 'double'),
                            exist_ok=True)
                plt.savefig(os.path.join('test-c16', 'output', 'all', '2D', f'lr_{lr}', f'ex_{exaggeration}', 'double',
                                         f'preplexity_{preplexity}.png'))
                plt.close()
                plt.figure()
                plt.scatter(x1[y == 0], x2[y == 0], s=3, c='b')
                plt.scatter(x1[y == 1], x2[y == 1], s=3, c='b')
                os.makedirs(os.path.join('test-c16', 'output', 'all', '2D', f'lr_{lr}', f'ex_{exaggeration}', 'single'),
                            exist_ok=True)
                plt.savefig(os.path.join('test-c16', 'output', 'all', '2D', f'lr_{lr}', f'ex_{exaggeration}', 'single',
                                         f'preplexity_{preplexity}.png'))
                plt.close()

    if args.PCA:
        pca = PCA(n_components=2)
        PCA_path = os.path.join('test-c16', 'output', 'PCA.png')
        pca_features = pca.fit_transform(x_means)
        ...
        fig = px.scatter_3d(df, x='x1', y='x2', z='x3', color='label')
        fig.write_html(os.path.join('test-c16', 'output', 'PCA.html'))
...
if __name__ == '__main__':
    ...
    if args.UMAP:
        try:
            ...
            raise Exception('unable to import package: umap, try venvumap')
        milnet = get_semitrans_milnet(args)
        plot_umap(args, milnet)
    elif args.t_SNE or args.PCA:
        milnet = get_semitrans_milnet(args)
        test_tSNE(args, milnet)
    else:
